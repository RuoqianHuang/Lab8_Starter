describe('Basic user flow for Website', () => {
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    const prodItems = await page.$$('product-item');
    // slack: The button of the first item has already been clicked in step 2. 
    // So by starting the loop from 0 in step 3, you are undoing this i.e removing it from the cart. 
    // That’s why it’s 19 instead of 20
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    await page.reload();
    const prodItems = await page.$$('product-item');
    let shadow, button, text, textVal;
    for (let i = 1; i < prodItems.length; i++) {
    ...
    let cartCount = await page.$('#cart-count');
    let carText = await cartCount.getProperty('innerText');
    let carTextVal = await carText.jsonValue();
    expect(carTextVal).toBe("20");
  }, 10000);
